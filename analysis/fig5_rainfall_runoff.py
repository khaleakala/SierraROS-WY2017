...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 31 18:31:24 2022

@author: kden

A 'conceptual experiment' comparing 'rainfall' (brightband above elevation) to 
discharge accumulations across winter periods and subbasins -- to illustrate
how runoff efficiency increases throughout winter; perhaps even the snowpack
being 'active' (if streamflow surpasses rainfall). 

Apply zero brightband bending here -- play with it in tinker.conceptual.extended.py
...
z0 = z0.loc[(z0.index>=d_wy1) & (z0.index<=d_wy2)]
...
qpe_end_times = pd.date_range(start = dt.datetime(2016,11,1,6, tzinfo=utc), 
                          end = dt.datetime(2017,5,7,0, tzinfo=utc), 
                          freq = '6H')
# ACTUALLY, LET'S EXPRESS IN START-OF-PERIOD TIME
qpe_times = pd.date_range(start = dt.datetime(2016,11,1,0, tzinfo=utc), 
                          end = dt.datetime(2017,5,6,18, tzinfo=utc), 
                          freq = '6H')
...
# 6-hourly bbh data set statistics 
# ovl --- n=664, R^2=0.933; SE=0.013 m; AIC/BIC = 1.001e+04
# cff --- n=671, R^2=0.955, SE=0.012 m; same AIC/BIC
...
# %% lastly -- a function to plot experiment results
...
def plot_multiple_accumulations(qflow6, qflowvar, qperain, d1, d2, ax, mycmap, legflag):
    ...
    qperain['bb_bend_0'].loc[
        (qperain.index>=d1) & (qperain.index<=d2)].cumsum().plot(
            ax=ax, x_compat=True, 
            color=cmap(0.4))

    if legflag==1:
        ax.legend(['Discharge', 'Rainfall'], loc='lower right',
...
# %% define dates for start times of each little "epoch"
# d1a = dt.datetime(2016,11,1, tzinfo=utc)
...
# ^ I changed a couple dates to bring the precip event to the front of the window
# this way, discharge doesn't get a "head start" over the rainfall

# %% functions to plot a worldview inset map onto each plot
# to map out the "antecedent snow cover"
...
# %% function to plot the season's experiment for a given stream gauge
...
plot_extended_results(gage_feather, shp_feather, dem_feather, 
                      qflow6_feather, 'ovl', [A1,A2,A3,A4], [0,0,0,1])
# --- plot Yuba 1
...
# %% annotate gauge numbers beneath maps
# date1 = dt.datetime(2016,11,16)
# date2 = dt.datetime(2016,12,30,12)
# date3 = dt.datetime(2017,1,31)
# date4 = dt.datetime(2017,3,17)
# A1.text(date1, 115, gage_feather, fontsize=13)
# A2.text(date2, 175, gage_feather, fontsize=13)
...
ofname += '5_conceptual_extended_v2.png'
